...
glRasterPos3f(-10,8,-20);
char clg[]="Vivekananda College of Engineering and Technology, Puttur";
for(int i=0;i<strlen(clg);i++)
    glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24,clg[i]);

      glRasterPos3f(-5,6,-20);
char project[]="Computer Graphics Project";
for(int i=0;i<strlen(project);i++)
    glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24,project[i]);

glRasterPos3f(-1,5,-20);
char project1[]="On";
for(int i=0;i<strlen(project1);i++)
    glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24,project1[i]);

glRasterPos3f(-4,4,-20);
char project2[]="'Brick Breaker Game'";
for(int i=0;i<strlen(project2);i++)
    glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24,project2[i]);


     glRasterPos3f(-20,-1,-20);
char guid[]="Under the Guidance of:";
for(int i=0;i<strlen(guid);i++)
    glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24,guid[i]);
    
    
    glRasterPos3f(-15,-2,-20);
char guidname[]="Prof.Krishna Mohana A J";
for(int i=0;i<strlen(guidname);i++)
    glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24,guidname[i]);
    
    glRasterPos3f(-15,-3,-20);
char guidname1[]="Prof.Ajay Shastry C G";
for(int i=0;i<strlen(guidname1);i++)
    glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24,guidname1[i]);

glRasterPos3f(-20,-8,-20);
char guid1[]="Instructions:";
for(int i=0;i<strlen(guid1);i++)
    glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24,guid1[i]);
    
glRasterPos3f(-15,-9,-20);
char guid2[]="Press A/a to Move Paddle to Left";
for(int i=0;i<strlen(guid2);i++)
    glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24,guid2[i]);

glRasterPos3f(-15,-10,-20);
char guid3[]="Press D/d to Move Paddle to Right";
for(int i=0;i<strlen(guid3);i++)
    glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24,guid3[i]);
    

       glRasterPos3f(6,-9,-20);
char key[]="Press Enter to continue...";
for(int i=0;i<strlen(key);i++)
    glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24,key[i]);

       glRasterPos3f(6,-10,-20);
char key2[]="Press S/s to Start the game...";
for(int i=0;i<strlen(key2);i++)
    glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24,key2[i]);


    glRasterPos3f(4,-1,-20);
char name[]="By:";
for(int i=0;i<strlen(name);i++)
    glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24,name[i]);

glRasterPos3f(6,-2,-20);
char name1[]="Devadatta Bhat	  - 4VP19CS024";
for(int i=0;i<strlen(name1);i++)
    glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24,name1[i]);

glRasterPos3f(6,-3,-20);
char name2[]="Dhanush Ghate D	 - 4VP19CS026";
